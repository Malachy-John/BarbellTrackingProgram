...
start_x, start_y, start_w, start_h = find_initial_coordinates('IMG_06_fix.MOV')
...
while True:
    ...
    for contour in contours:
        ...
        if area > 500:
            ...
            ref_radius = radius
            end_point = (center[0] + radius, center[1])  # Moving rightwards from the center

            cv2.line(frame, center, end_point, (0, 255, 0), 2)  # Green line
            ...
            if set_started and not concentric_started:
                if y_disp < -2:
                    ...
                    if distance_check < -80:
                        print(distance_check)
                        rep_ending_y_pos = center[1] + int(ref_radius * 1.5)           
                ...
                if tracked_y <= rep_ending_y_pos:
                    ...
                    metres_per_second = distance_metres / rep_duration_s

                    print(f"Your m/s value is: {metres_per_second}")

            cv2.putText(frame, f"Repetitions: {rep_count}", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                (0, 255, 0), 2)
            

            rep_start_path = (end_x, end_y)  # Starting at the left edge of the image
            rep_end_path = (finish_x, finish_y)  # Ending at the right edge of the image
